  ...
  function displayFavorites () {
    ...
    favMovies.forEach((movie) => {
      ...
      movieFavCard.innerHTML = `
          <img src="https://image.tmdb.org/t/p/original${movie.poster_path ? movie.poster_path:movie.backdrop_path}" alt="${
        movie.title
      }">
          <h3>${movie.title}</h3>
...
async function loadHeroBanner () {
  ...
  movies.forEach((movie, index) => {
    ...
    slide.innerHTML = `
      <img src="${IMG_PATH}${movie.backdrop_path? movie.backdrop_path:movie.poster_path}" alt="${movie.title}" onclick="window.location.href = 'src/preview.html'">
